...
public class HTPlatformController : HTRaycastController {
	...
	private Dictionary<Transform,HTController2D> passengerDictionary = new Dictionary<Transform, HTController2D>();
	...
	private void MovePassengers(bool isBeforeMovePlatform) {
		foreach (PassengerMovement passenger in this.passengerMovement) {
			if (!this.passengerDictionary.ContainsKey(passenger.transform)) {
				this.passengerDictionary.Add(passenger.transform, passenger.transform.GetComponent<HTController2D>());
			}
